...
def get_sorted_images(
    path,
):  # Question: should this not be its own function but instead just carried out in __init__ of our Class?
    """
...
def analyze_scores(scores):
    ...
    for (
        ...
        scores_dict.items()
    ):  # ascending indices order for images, label if an image suffer from an issue
        if (v2 - mean) / stdev < -2:
            issue_bool[k2] = 1
        else:
            issue_bool[k2] = 0
