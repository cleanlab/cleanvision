...
def calc_image_size(image: Image) -> int:
    size = image.size
...
class SizeProperty(ImageProperty):
    ...
    def calculate(self, image: Image) -> Dict[str, int]:
        return {self.name: calc_image_size(image)}
    ...
    def get_scores(
        ...
        **kwargs: Any,
        ...
        assert raw_scores is not None
        scores = pd.DataFrame(index=raw_scores.index)
        scores[get_score_colname(issue_type)] = raw_scores[issue_type].apply(lambda x: x / raw_scores[issue_type].median())
        print(scores.head())
        return scores
