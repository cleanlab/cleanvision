...
class Imagelab:
    ...
    def find_issues(self, issue_types=None):
        ...
        print(f"Checking for {', '.join([issue_type.value for issue_type in self.issue_types])} images")
        ...
        for issue_manager in self.issue_managers:
            ...
            self.issues = self.issues.merge(issue_manager.issues, how='left', on='image_path')
    ...
    def _set_issue_managers(self):
        ...
        for issue_type in self.issue_types:
            if issue_type.property:
                ...
                self.issue_managers.append(_IssueManagerFactory.from_str(issue_type.value))
        if len(image_property_issues) > 0:
            self.issue_managers.append(_IssueManagerFactory.from_str("ImageProperty")(image_property_issues))
    ...
    def _visualize(self, topk=5):
        ...
        for issue_type in topk_issues:
            ...
            sorted_filepaths = sorted_df['image_path'].head(num_images_per_issue).tolist()
            image_paths.extend(sorted_filepaths)

        nrows, ncols = int(len(image_paths) / num_images_per_issue), num_images_per_issue

        fig, axes2d = plt.subplots(nrows=max(nrows, 2), ncols=ncols,
